...
<%
	...
	//페이징
	BoardListsTO anListsTO = (BoardListsTO)request.getAttribute("anListsTO");
	ArrayList<BoardTO> anLists = anListsTO.getBoardLists();
	// 현재페이지
	//int cpage = Integer.valueOf(request.getParameter("cpage"));
	int cpage = anListsTO.getCpage();
	// 한 페이지당 출력 데이터 개수
	int recordPerPage = anListsTO.getRecordPerPage();	
	// 전체 데이터(글) 개수
	int totalRecord = anLists.size();
	// 전체 페이지 개수 = 마지막 페이지
	int totalPage = ( ( (totalRecord - 1) / recordPerPage ) + 1 );
	// 페이지번호가 몇개씩 보이게 할지 설정
	int blockPerPage = anListsTO.getBlockPerPage();
	// 보이는 페이지 번호의 시작부분이다.
	...
	if (anLists.size() == 0) {
		...
		for ( int i=0; i < recordPerPage; i++) {
			...
			BoardTO boardTO = anLists.get(i);
			if( cpage != 1 ){	
				boardTO = anLists.get(i+((cpage-1)*10));
			}
...
<div id="Container">
	...
	function bbsModify(seq) {
		window.open("admin_contents_modify.mysql?cpage=1&seq=" + seq, "getBbs1", "width=800,height=850,resizable=no,scrollbars=yes, left=50, top=50");				
	}
	
	function an_write(seq) {
		window.open("admin_an_write.mysql?cpage=1", "getBbs1", "width=800,height=850,resizable=no,scrollbars=yes, left=50, top=50");				
	}
	...
	function deleteBbs(seq){
		if(confirm('선택한 게시글을 삭제하시겠습니까?\n\n삭제한 데이타는 복구할수 없습니다.')){
			...
			document.location = './admin_an_delete.mysql?cpage=<%=cpage%>&seq='+seq;
		}
	}
...
<div id="location">HOME > 고객센터 관리 > 공지사항 관리</div>
<div id="S_contents">
<h3>공지사항  관리</h3>	 

<form name="frm" action="admin_an.mysql" method="get">
		...
		<tbody>
			...
			<div class="bbs_button top10">
				<div class="bbs_btn_left">
					...
					<button type='button' class='h22 t4 small icon gray' onClick="delBbsNewBbs('admin_an_multidelete.mysql', '<%=cpage %>');"><span class='icon_x'></span>선택삭제</button>
				</div>
				<div class="bbs_btn_right">
		    		<button type="button" class="h22 t4 small icon gray" onClick='an_write();'><span class='icon_check'></span>쓰기</button>
		    	</div>
			</div>      
			<div class="top5">
