...
<%
	...
	//페이징
	BoardListsTO commentListsTO = (BoardListsTO)request.getAttribute("commentLists");
	//결과값 가져오기
	ArrayList<BoardTO> commentLists = commentListsTO.getBoardLists();
	
	// 현재페이지
	//int cpage = Integer.valueOf(request.getParameter("cpage"));
	int cpage = commentListsTO.getCpage();
	// 한 페이지당 출력 데이터 개수
	int recordPerPage = commentListsTO.getRecordPerPage();	
	// 전체 데이터(글) 개수
	int totalRecord = commentLists.size();
	// 전체 페이지 개수 = 마지막 페이지
	int totalPage = ( ( (totalRecord - 1) / recordPerPage ) + 1 );
	// 페이지번호가 몇개씩 보이게 할지 설정
	int blockPerPage = commentListsTO.getBlockPerPage();
	// 보이는 페이지 번호의 시작부분이다.
	...
	if (commentLists.size() == 0) {
		...
		sbHTML.append("<td></td>");
		sbHTML.append("<td></td>");		
		sbHTML.append("<td align=\"center\">등록 된 댓글이 없습니다.</td>");
		sbHTML.append("<td></td>");
		sbHTML.append("<td></td>");
		sbHTML.append("<td></td>");
		sbHTML.append("<td></td>");
		...
		for ( int i=0; i < recordPerPage; i++) {
			...
			if( cpage != 1 ){	
				commentTO = commentLists.get(i+((cpage-1)*10));
			}
			
			seq = commentTO.getSeq();
			cseq = commentTO.getCseq();
			String comment = commentTO.getComment();
			String cwriter = commentTO.getCwriter();
			String cwdate_ori = commentTO.getCwdate_ori();
			String pseq_kind = commentTO.getPseq_kind();
			//sbHTML.append("");
			
			sbHTML.append("<tr>");
			sbHTML.append("<td align=\"center\"><form style='margin:0;'><input type='hidden' name='idx' value='"+cseq+"'><input type='checkbox' name='select_checkbox'></form></td>");
			sbHTML.append("<td width=\"5%\">"+ cseq +"</td>");
			sbHTML.append("<td width=\"10%\"> ["+pseq_kind+"] "+ seq +"번글</td>");
			sbHTML.append("<td style=\"text-align:left;\"><a href=\"javascript:bbsView('"+seq+"');\" style=\"cursor:point\" > "+ comment +" </a></td>");
			sbHTML.append("<td width=\"10%\">"+ cwriter +"</td>");
			sbHTML.append("<td width=\"8%\">"+ cwdate_ori +"</td>");
			sbHTML.append("<td align=\"center\"><a onclick=\"bbsModify('"+seq+"', '"+cseq+"');\"><button type=\"button\" class=\"h18 t3 color small round black_s\">수정</button></a></td>");
			sbHTML.append("<td align=\"center\"><button type=\"button\" class=\"h18 t3 color small round red_s\" onClick=\"deleteBbs('"+cseq+"');\">삭제</button></td>");
			sbHTML.append("</tr>");
	...
	<ul id="Lnb">
		<li class="on"><a href="./admin_contents.mysql?cpage=1&codeopt=1&sdate=&edate=&searchopt=&searchkey=" onFocus="this.blur();">게시판 관리</a>
			<ul>
				...
				<li>
					<a href="./admin_comments.mysql?cpage=1&codeopt=1&sdate=&edate=&searchopt=&searchkey=">댓글통합관리</a>
				</li>				
			</ul>
		</li>		
	</ul>
...
<!-- //left_area// -->
	...
	/* function excelDown(){
		...
		document.location = "bbs_manage_excel.php?searchopt=&searchkey=&codeopt=&sdate=&edate=";

	}
	
	function commentModify(idx, code) {
		window.open("bbs_comment.php?mode=modify&idx=" + idx + "&code=" + code, "getBbs", "width=600,height=300,resizable=yes,scrollbars=yes, left=200, top=200");				
	}
	function bbsReply(idx, code) {
		window.open("bbs.php?ptype=input&mode=reply&idx=" + idx + "&code=" + code, "getBbs", "width=800,height=850,resizable=yes,scrollbars=yes, left=50, top=50");				
	} */
	
	function bbsModify(seq,cseq) {
		window.open("admin_comments_modify.mysql?cpage=1&seq=" + seq + "&cseq=" + cseq, "getBbs1", "width=850,height=850,resizable=no,scrollbars=yes, left=50, top=50");				
	}
	...
	function deleteBbs(cseq){
		if(confirm('선택한 게시글을 삭제하시겠습니까?\n\n삭제한 데이타는 복구할수 없습니다.')){
			//document.location = './admin_comments_delete.mysql';
			document.location = './admin_comments_delete.mysql?cpage=<%=cpage%>&cseq='+cseq;
		}
	}
 ...
 <div id="location">HOME > 댓글 통합관리</div>
<div id="S_contents">
<h3>댓글 통합관리</h3>	  

	<form name="frm" action="admin_comments.mysql" method="get">
	...
	<tr>
		...
		<td class="tdlast">
			<select name="searchopt" class="select w80">
				<option value="comment" >댓글내용
				<option value="cwriter" >작성자
			</select>
	...
	<div class="top15 center"><button type="button" onClick='search()' class="b h28 t5 color blue_big">검색하기</button></div>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="bbs_basic_list">
		...
		<tbody>
			<%=sbHTML %>
			<!-- <tr>
				<td align="center"><form style='margin:0;'><input type='hidden' name='idx' value='5318'><input type='checkbox' name='select_checkbox'></form></td>
				<td width="5%">번호</td>
				<td width="7%">댓글위치</td>
				<td style="text-align:left;"><a href="javascript:bbsView('5318', 'notice');" style="cursor:point" ><b>[댓글]</b> 댓글 테스트 입니다. </a></td>
				<td width="10%">작성자</td>
				<td width="8%">작성일</td>
				<td width="8%">IP</td>
				<td align="center"><a onclick="bbsModify('5318', 'notice');"><button type="button" class="h18 t3 color small round black_s">수정</button></a></td>
				<td align="center"><button type="button" class="h18 t3 color small round red_s" onClick="deleteBbs('notice', '5318');">삭제</button></td>
        	</tr> -->
		</tbody>
      </table>
								...
								<tr>
									<%
										...
										if( cpage <= blockPerPage ){											
											...
											/* out.println("<a href='admin_comments.mysql?cpage=1&codeopt="+codeopt+"&sdate="+sdate
											...
											out.println("<td width='22' height='50'>");
											out.println("<a href='admin_comments.mysql?cpage="+(cpage-5)+"&codeopt="+codeopt+"&sdate="+sdate
													+"&edate="+edate+"&searchopt="+searchopt+"&searchkey="+searchkey+"'>");
											out.println("<img src='./resources/assets/img/admin/btn_prev2.gif' align='absmiddle' border=0'>");
											out.println("</a>");
											out.println("</td>");
										...
										if( cpage == 1 ){																			
											...
											out.println("<td width='22'>");
											out.println("<a href='admin_comments.mysql?cpage="+(cpage-1)+"&codeopt="+codeopt+"&sdate="+sdate
													+"&edate="+edate+"&searchopt="+searchopt+"&searchkey="+searchkey+"'>");
											out.println("<img src='./resources/assets/img/admin/btn_prev.gif' align='absmiddle' border=0'>");
											out.println("</a>");
											out.println("</td>");										
										...
										for(int i=startBlock; i<=endBlock; i++){
											if( cpage == i ){
												//out.println("<b>["+i+"]</b>");
												out.println("<a href='admin_comments.mysql?cpage=" + i + "&codeopt="+codeopt+"&sdate="+sdate
										...
										if( cpage == totalPage ){
											...
											/* out.println("<a href='admin_comments.mysql?cpage="+totalPage+"&codeopt="+codeopt+"&sdate="+sdate
													+"&edate="+edate+"&searchopt="+searchopt+"&searchkey="+searchkey+"'>");
										...
										if( cpage/blockPerPage >= 0 ){											
											...
											out.println("<td width='22' align='right'>");
											out.println("<a href='admin_comments.mysql?cpage="+(startBlock+blockPerPage)+"&codeopt="+codeopt+"&sdate="+sdate
													+"&edate="+edate+"&searchopt="+searchopt+"&searchkey="+searchkey+"'>");
											out.println("<img src='./resources/assets/img/admin/btn_next2.gif' align='absmiddle' border='0'>");
											out.println("</a>");
											out.println("</td>");
